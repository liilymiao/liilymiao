...
The heatmap shows 52 Sunday-first calendar columns ending in the source statistics week. Future dates are omitted. Rainbow hue progresses from warm red through yellow, green, and blue to violet across the visible active dates. Hue is decorative, not a token-count scale. Four opacity levels encode daily usage: 0–25%, over 25–50%, over 50–75%, and over 75% of the maximum daily count in the displayed period, using opacities 0.38, 0.58, 0.79, and 1. Only active days receive color; days absent from the source's sparse history remain neutral, matching the app's zero-day handling. The README contains only the light/dark card; all source and update details live in this document.
